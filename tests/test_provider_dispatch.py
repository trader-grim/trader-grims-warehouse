...
def test_foreman_worker_and_receipt_use_local_runner_and_preserve_plan_binding(tmp_path, monkeypatch):
    ...
    binding = {
        ...
        "source_commit": "a" * 40,
        "requested_worktree_identity": "allocated",
    ...
    with (
        patch("tgw.development.foreman.validated_coding_worktree", return_value=worktree),
...
def test_malformed_selected_provider_adapter_refuses_before_enqueue(tmp_path):
    ...
    with pytest.raises(ProviderDispatchError, match="no eligible|mapping is malformed"):
        resolve_implementation_adapter(
            ...
            registry_path=bad_catalog,
            adapters=_adapters(),
        )


def test_local_worker_does_not_propagate_legacy_provider_adapter_fields(tmp_path, monkeypatch):
    worktree = tmp_path / "worktree"
    worktree.mkdir()
    worker = CodingWorker(
        "codex-implement",
    ...
    receipt = worker.handle(
        {
            "payload_json": {
                ...
                "graph_id": "g",
                "object_generation": "v",
                "coding_role": "implementation",
                "selected_provider": "wrong",
                "adapter_treatment_id": "codex-implement",
                "adapter_queue_name": "codex-implement",
            }
        }
    )
    assert not {"coding_role", "selected_provider", "adapter_treatment_id", "adapter_queue_name"} & receipt.keys()
    worker._launcher.assert_called_once()
