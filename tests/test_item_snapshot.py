...
def test_staged_content_current_uses_only_authoritative_receipt_lookup(tmp_path):
    path = tmp_path / "item.json"
    item = _make_item(ebay_offer={"offer_id": "offer-1"}, ebay_listing={})
...
def test_published_listing_supersedes_staged_content_freshness(tmp_path):
    path = tmp_path / "item.json"
    item = _make_item(
    ...
    path.write_text(json.dumps(item), encoding="utf-8")
    goal = GoalProfile("listed", "1", ("staged_content_current", "published"))
