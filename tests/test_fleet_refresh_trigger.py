...
def test_watched_revision_creates_one_durable_coalesced_request(tmp_path):
    ...
    assert json.loads((tmp_path / "state/pending-refresh.json").read_text())[
        "request_hash"
    ] == first["request_hash"]


def test_changed_watched_revision_creates_a_new_exact_request(tmp_path):
    config = _config(tmp_path)
    first = trigger_configured_fleet_refresh(config)
    watched = tmp_path / "watched.json"
...
def test_trigger_refuses_tmp_or_malformed_watched_input(tmp_path):
    config = _config(tmp_path)
    ...
    watched = tmp_path / "second/watched.json"
    watched.write_text("{}")
