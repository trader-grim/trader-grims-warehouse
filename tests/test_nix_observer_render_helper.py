...
def test_observer_module_declares_the_canonical_unit_set_without_duplicating_the_production_flake():
    source = Path("nix/nix-input-observer-launcher.nix").read_text()
    assert 'systemd.slices."tgw-nix-input-observer"' in source
    assert "systemd.sockets.tgw-nix-input-observer" in source
    assert 'systemd.services."tgw-nix-input-observer@"' in source
    assert "packages.${system}" not in Path("flake.nix").read_text()
...
def test_production_git_authority_is_closed_without_a_mutable_source_tree_receipt():
    assert helper.PRODUCTION_GIT_AUTHORITY == helper.ToolAuthority(
        authority_receipt_sha256=helper.AUTHORIZED_TOOL_RECEIPT_SHA256,
        path=helper.AUTHORIZED_GIT_PATH,
        sha256=helper.AUTHORIZED_GIT_SHA256,
        bytes=helper.AUTHORIZED_GIT_BYTES,
        owner_uid=helper.AUTHORIZED_GIT_OWNER_UID,
        mode=helper.AUTHORIZED_GIT_MODE,
    )


def test_production_a2_authority_cross_binds_both_admitted_receipt_hashes():
    assert helper.PRODUCTION_A2_AUTHORITY.a1_prerequisite_receipt_sha256 == helper.AUTHORIZED_TOOL_RECEIPT_SHA256
    assert helper.PRODUCTION_A2_AUTHORITY.a2_prerequisite_receipt_sha256 == helper.AUTHORIZED_RENDER_RECEIPT_SHA256
    assert helper.PRODUCTION_A2_AUTHORITY.input_path == helper.AUTHORIZED_INPUT_PATH
    assert helper.PRODUCTION_A2_AUTHORITY.input_nar_sha256 == helper.AUTHORIZED_INPUT_NAR_SHA256
    assert helper.PRODUCTION_A2_AUTHORITY.systemd_analyze_sha256 == helper.AUTHORIZED_SYSTEMD_ANALYZE_SHA256
