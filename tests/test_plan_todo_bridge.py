...
def test_explicit_pp_root_uses_actual_pp_reference_and_supersedes():
    solution, compiled = _compiled(root_id="PP-WORKFLOW-001"); rows = []
    calls = []
    ...
    first = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="one", agent="codex", body="x", priority=1, create_todo=create, list_todos=lambda: rows, allocate_worktree=allocator, set_status_note=note, execution_root=root)
    again = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="one", agent="codex", body="x", priority=1, create_todo=create, list_todos=lambda: rows, allocate_worktree=allocator, set_status_note=note, execution_root=root)
    second = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="two", agent="codex", body="x", priority=1, create_todo=create, list_todos=lambda: rows, allocate_worktree=allocator, set_status_note=note, execution_root=root)
...
def test_explicit_todo_root_reuses_canonical_todo_without_duplicate():
    solution, compiled = _compiled(); rows = [{"id": 41, "status_note": ""}]
    creates = MagicMock()
    def note(todo_id, value): rows[0]["status_note"] = value
    result = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="selected", agent="codex", body="x", priority=1, create_todo=creates, list_todos=lambda: rows, allocate_worktree=lambda todo_id, request_id, source: {"worktree": f"/worktrees/{request_id}", "todo_id": todo_id}, set_status_note=note, execution_root={"schema": "tgw-execution-root/v1", "kind": "todo", "todo_id": 41})
    again = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="selected", agent="codex", body="x", priority=1, create_todo=creates, list_todos=lambda: rows, allocate_worktree=lambda *_: {}, set_status_note=note, execution_root={"schema": "tgw-execution-root/v1", "kind": "todo", "todo_id": 41})
...
def test_mismatched_or_fake_execution_root_refuses_closed():
    ...
    common = dict(solution=solution, treatment_id="establish:base@1", source_commit="a" * 40, worktree_identity="x", agent="codex", body="x", priority=1, create_todo=lambda *_: {}, list_todos=lambda: [], allocate_worktree=lambda *_: {}, set_status_note=lambda *_: None)
    for root in (
    ...
    with pytest.raises(PlanTodoBridgeError, match="integrity"):
        bind_leaf(
            compiled, execution_root={"schema": "tgw-execution-root/v1", "kind": "pp", "pp_ref": "PP-WORKFLOW-001"},
...
def test_bridge_todo_foreman_payload_and_execution_envelope_retain_plan_binding(tmp_path, root_id, execution_root, existing_id):
    ...
    result = bind_leaf(compiled, solution=solution, treatment_id="establish:base@1", source_commit="a"*40, worktree_identity="allocated", agent="codex", body="implement", priority=1, create_todo=create, list_todos=lambda: rows, allocate_worktree=lambda *_: {**location, "todo_id": _[0]}, set_status_note=note, execution_root=execution_root)
