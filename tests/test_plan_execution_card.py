...
from tgw.plan_execution_card import PlanExecutionCardError, build_execution_card, select_next_execution_card, validate_execution_card
...
def _resolved():
    document = {
        "schema": "tgw-plan/v2",
        "plan_commit": COMMIT,
        "capabilities": ["one@1", "two@1", "promptcraft.receiver-profiles@1"],
        "providers": [
            {"id": "one", "provides": ["one@1"]},
            {"id": "two", "provides": ["two@1"], "requires": ["one@1"]},
            {"id": "recovered-promptcraft", "provides": ["promptcraft.receiver-profiles@1"]},
        ],
        "observations": [],
...
def test_card_requires_all_registered_resource_bindings_and_exact_solution():
    solution, compiled = _resolved()
    resources = _resources()
    resources.pop("codegraph_snapshot")
    with pytest.raises(PlanExecutionCardError, match="resources"):
        build_execution_card(
            compiled=compiled,
            solution=solution,
            execution_graph=_graph(),
            treatment_id="establish:one@1",
            source_commit=SOURCE,
            source_tree=TREE,
            resources=resources,
            environment={"id": "dev"},
...
def test_execution_card_is_not_a_plan_todo_transport_input():
    ...
    def create(agent, body, priority, *_):
        row = {"id": 1, "status_note": "", "body": body, "priority": priority}
        rows.append(row)
        return row
    ...
    result = bind_leaf(
        ...
        treatment_id="establish:one@1",
        source_commit=SOURCE,
        worktree_identity="request",
        agent="codex",
        body="implement one",
        priority=7,
        create_todo=create,
        list_todos=lambda: rows,
        allocate_worktree=lambda *_: {"worktree": "/worktrees/test"},
    ...
    with pytest.raises(TypeError, match="execution_envelope"):
        bind_leaf(
            ...
            treatment_id="establish:one@1",
            source_commit=SOURCE,
            worktree_identity="second",
            agent="codex",
            body="implement one",
            priority=7,
            execution_envelope={"legacy": True},
            create_todo=create,
            list_todos=lambda: rows,
            allocate_worktree=lambda *_: {"worktree": "/worktrees/test2"},
            set_status_note=note,
            execution_root=_root(),
        )
...
def test_stale_or_mismatched_card_refuses_closed():
    ...
    stale["plan"]["commit"] = "d" * 40
    with pytest.raises(PlanExecutionCardError, match="hash"):
        validate_execution_card(stale, compiled=compiled)
    mismatched = copy.deepcopy(card)
    mismatched["work_unit"]["treatment_id"] = "establish:two@1"
    mismatched["card_hash"] = "sha256:" + "0" * 64
...
def test_next_eligible_card_selection_is_phase_then_provider_deterministic():
    ...
    selected = select_next_execution_card(
        compiled=compiled, solution=solution, execution_graph=_graph(), source_commit=SOURCE, source_tree=TREE, resources=_resources(), environment={"id": "dev"}, execution_root=_root()
    )
    again = select_next_execution_card(
        compiled=compiled, solution=solution, execution_graph=_graph(), source_commit=SOURCE, source_tree=TREE, resources=_resources(), environment={"id": "dev"}, execution_root=_root()
    )
