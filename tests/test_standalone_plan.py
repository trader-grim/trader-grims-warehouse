...
def test_parent_program_is_not_misrepresented_as_bootstrap_completion():
    ...
    parent = (
        root / "tests/fixtures/workflow_completable_plan/environment-cleanup-program.md"
    ).read_text()
...
def test_environment_cleanup_bootstrap_fixture_is_workflow_completable():
    ...
    plan = parse_plan(
        (
            root
            / "tests/fixtures/workflow_completable_plan/environment-cleanup-workflow-bootstrap.md"
        ).read_text()
    )
    validate_plan(plan, registry)
