...
def _receipt():
    return EffectExecutionReceipt(
        schema="tgw-effect-execution-receipt/v1",
        request_id="w09",
        authority_receipt_id="authority:1",
        effect_hash="sha256:" + "1" * 64,
        effect_kind="approval-platform-bootstrap-deployment",
        generation="release-b",
        handler_id="governed-application-bootstrap-install@2",
...
def _store(tmp_path):
    ...
    return ImmutableEffectCompletionStore(
        root,
        sink_id="w09-terminal",
        descriptor_hash=completion_store_descriptor_hash(
            root,
            sink_id="w09-terminal",
...
def test_store_loops_short_writes_and_rejects_hardlinked_existing_artifact(tmp_path, monkeypatch):
    ...
    source.write_bytes(__import__("json").dumps(receipt, sort_keys=True, separators=(",", ":")).encode() + b"\n")
    source.chmod(0o400)
    os.link(source, other_root / name)
    with pytest.raises(OSError, match="readback mismatch"):
