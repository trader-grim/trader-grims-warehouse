...
from promptcraft.core import compare_prompts, compile_prompt, harness_profile, lint_prompt
from promptcraft.server import _handle
...
class PromptComparisonTests(unittest.TestCase):
    def test_comparison_identifies_strategy_improvement(self):
        result = compare_prompts(
            ...
            new_prompt="Use code execution to index and search sources. Do not load the entire corpus into context. Return headings exactly once: VERDICT, EVIDENCE. Cite each source fact with a source path and line range. Return the complete deliverable now.",
            harness="antigravity-managed",
            source_paths=["sources/plan.md"],
            declared_tools=["code_execution"],
        )
