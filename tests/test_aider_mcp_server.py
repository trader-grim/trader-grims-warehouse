...
def test_run_task_empty_files():
    result = json.loads(ams.aider_run_task('do something', []))
    assert result['ok'] is False
    assert 'empty' in result['error']


def test_run_task_traversal_rejected():
    result = json.loads(ams.aider_run_task('do something', ['../../bad.py']))
    assert result['ok'] is False
...
def test_run_task_success(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Add qty guard', ['src/tgw/items.py']))
...
def test_run_task_architect_mode(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Refactor X', ['src/tgw/api.py'], mode='architect'))
    assert result['ok'] is True
...
def test_run_task_no_changes(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Make it better', ['src/tgw/items.py']))
...
def test_run_task_aider_failure(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Break things', ['src/tgw/items.py']))
    assert result['ok'] is False
...
def test_run_task_timeout(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Something slow', ['src/tgw/items.py']))
    assert result['ok'] is False
...
def test_run_task_output_truncated(tmp_path, monkeypatch):
    ...
    result = json.loads(ams.aider_run_task('Something big', ['src/tgw/items.py']))
