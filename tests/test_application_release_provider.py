...
def _descriptor(parameters=None):
    ...
    value = {
        ...
        "target": {
            "host": "tgw-prod",
            "address": "100.107.99.66",
            "port": 22,
            "user": "db",
        },
        "transport": {
            "ssh_path": "/run/current-system/sw/bin/ssh",
            "ssh_sha256": h("1"),
            "ssh_keygen_path": "/run/current-system/sw/bin/ssh-keygen",
            "ssh_keygen_sha256": h("9"),
            "known_hosts_path": "/etc/tgw/tgw-prod.known_hosts",
            "known_hosts_sha256": h("2"),
            "identity_path": "/etc/tgw/w09.key",
            "identity_sha256": h("3"),
        ...
        "candidate": {
            "archive_path": "/opt/TGW/releases/candidate.tar",
            "archive_sha256": h("5"),
            "commit": "a" * 40,
            "tree": "b" * 40,
        ...
        "remote_boundary": {
            "python_path": "/nix/store/00000000000000000000000000000000-python3-3.13.5/bin/python3.13",
            "python_sha256": h("b"),
            "sudo_path": "/run/wrappers/wrappers.ABC123/sudo",
            "sudo_sha256": h("c"),
            "bootstrap_sha256": h("d"),
            "config_path": "/etc/tgw/w09-memory-config.json",
            "config_sha256": h("7"),
        ...
        "prerequisite_receipt": {
            "ref": "w09-prerequisite:1",
            "path": "/etc/tgw/w09-prerequisite.json",
            "sha256": h("8"),
        },
    }
    ...
    boundary["remote_command_sha256"] = _hash_bytes(
        shlex.join(
            [
                REMOTE_SUDO,
                "-n",
...
def test_descriptor_requires_fixed_tailnet_address_and_full_parameter_binding():
    ...
    with pytest.raises(ApplicationReleaseProviderError, match="mounted provider"):
        SshApplicationReleaseProvider._check_parameters(
            provider,
            {**parameters, "migrations": ["neighbor"]},
        )
...
def test_remote_response_schema_rejects_untyped_or_empty_evidence():
    response = {
        "schema": "tgw-w09-application-release-response/v1",
        "operation_id": "w09-a",
        "helper_sha256": "sha256:" + "1" * 64,
        "helper_config_sha256": "sha256:" + "2" * 64,
        "nix_system_path": "/nix/store/system",
        "status": "SUCCEEDED",
        "evidence": ["health:exact"],
...
def test_memory_bootstrap_is_digest_bound_and_registers_modules_before_exec():
    ...
    bootstrap = _memory_bootstrap(
        installer_sha256=h("1"),
        transaction_sha256=h("2"),
        helper_config_sha256=h("3"),
        python_sha256=h("4"),
        sudo_sha256=h("5"),
...
def test_memory_bootstrap_executes_exact_framed_modules_without_persistence(monkeypatch):
    ...
    transaction = b"import sys\ndef memory_main(request, config, archive, runtime, helper_sha256):\n    sys.stdout.buffer.write(b'framed-ok')\n    return 0\n"
    ...
    bootstrap = _memory_bootstrap(
        ...
        python_sha256=_hash_bytes(python_raw),
        sudo_sha256=_hash_bytes(python_raw),
        python_path=python,
        sudo_path=python,
        python_stat={
            "uid": python_stat.st_uid,
            "gid": python_stat.st_gid,
            "mode": python_stat.st_mode & 0o7777,
    ...
    unsigned = {
        "schema": provider_module.FRAME_SCHEMA,
