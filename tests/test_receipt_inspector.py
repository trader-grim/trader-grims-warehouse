...
def test_list_and_show_verify_claimed_receipt_hash(tmp_path):
...
def test_forged_hash_is_reported_without_trusting_status(tmp_path):
    receipt = {"schema": "example/v1", "status": "PASS", "receipt_hash": "sha256:" + "0" * 64}
    (tmp_path / "forged.json").write_text(json.dumps(receipt))
...
def test_unregistered_traversal_symlink_and_tmp_roots_fail_closed(tmp_path):
    ...
    with pytest.raises(ReceiptInspectionError):
        inspect_receipt(_config(tmp_path), root_id="missing", receipt_id="one")
    with pytest.raises(ReceiptInspectionError):
        inspect_receipt(_config(tmp_path), root_id="fleet", receipt_id="../outside")
    with pytest.raises(ReceiptInspectionError):
        inspect_receipt(_config(tmp_path), root_id="fleet", receipt_id="linked")
    with pytest.raises(ReceiptInspectionError):
        list_receipts(_config("/tmp"), root_id="fleet")


def test_invalid_json_is_visible_in_list_and_refused_for_show(tmp_path):
    (tmp_path / "broken.json").write_text("{")
    listed = list_receipts(_config(tmp_path), root_id="fleet")
    assert listed["receipts"][0]["status"] == "INVALID"
    with pytest.raises(ReceiptInspectionError):
        inspect_receipt(_config(tmp_path), root_id="fleet", receipt_id="broken")
