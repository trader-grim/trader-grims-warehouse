...
def test_set_fields_records_durable_mutation_receipt(cfg):
    """Phase 1 records local durable evidence and invokes no external audit."""
    items.create_item(cfg, 'tgw1', {'title': 't'})
    items.set_fields(cfg, 'tgw1', {'ebay_category_id': '123', 'ebay_category_name': 'Books'})
    journal = cfg['itemdata_root'].parent / '.item-mutations' / 'receipts.jsonl'
    events = [json.loads(line) for line in journal.read_text().splitlines()]
    result = [e['result'] for e in events if e.get('event') == 'terminal'][-1]
    assert set(result['changes']) == {'ebay_category_id', 'ebay_category_name'}
