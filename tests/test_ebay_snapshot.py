...
def test_photo_check_skipped_when_recently_verified(monkeypatch):
    ...
    item['ebay_listing']['photo_verify'] = {
        ...
        'submitted_count': 2, 'confirmed_count': 2,
    }
    worker = _worker(_cfg())
...
def test_photo_check_runs_when_overdue(monkeypatch):
    ...
    item = _item_with_submitted(['u1', 'u2'])
    item['ebay_listing']['photo_verify'] = {
        'verified_at': old_ts, 'submitted_count': 2, 'confirmed_count': 2,
    }

    monkeypatch.setattr(ebay_sync_mod, 'ebay_get',
                        lambda *a, **k: {'product': {'imageUrls': ['u1', 'u2']}})
...
def test_photo_check_runs_when_never_verified(monkeypatch):
    ...
    monkeypatch.setattr(ebay_sync_mod, 'ebay_get',
                        lambda *a, **k: {'product': {'imageUrls': ['u1']}})
...
def test_repush_not_enqueued_when_counts_match(monkeypatch):
    ...
    enqueued = []
    monkeypatch.setattr(ebay_sync_mod.state_machine, 'enqueue_job',
                        lambda **k: enqueued.append(k) or 1)
