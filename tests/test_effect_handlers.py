...
class _AuthorityStore:
    ...
    def __init__(self, *, receipt_id="authority:1", begin_error=None, complete_error=None):
        self.begin_execution = Mock(
            return_value={"receipt_id": receipt_id},
            side_effect=begin_error,
        )
...
class _BootstrapContractResolver:
    ...
    def __init__(self):
        self.resolve = Mock(
            return_value=VerifiedBootstrapDeploymentContract(
...
def test_release_failure_invokes_only_registered_rollback_and_receipts_it():
    ...
    authority.complete_execution.assert_called_once_with(
        ...
        outcome="rolled_back",
        evidence=(),
        rollback_receipt="rollback:exact",
...
def test_bootstrap_consumption_ambiguity_is_persisted_as_a_terminal_controller_receipt():
    registry, providers = _registry()
    effect = TypedEffect.parse(
        {
            "kind": "dependency-resubmit",
            "generation": "g2",
            "parameters": {
                "dependency_id": "W",
                "queue_id": "coding",
    ...
    authority_evidence = ("bootstrap-consumption-ambiguity:sha256:" + "e" * 64,)

    def ambiguous_authority(*_args, **_kwargs):
        raise BootstrapConsumptionAmbiguous(
            "bootstrap consumption persistence is ambiguous",
            evidence=authority_evidence,
            cause=OSError("fsync failed"),
        )

    terminal = Mock(
        side_effect=lambda receipt: {
            "receipt": "terminal:ambiguity",
    ...
    receipt = AuthorityEffectController(
        registry,
        ambiguous_authority,
        terminal_recorder=terminal,
        bound_evidence=("controller-runtime:sha256:" + "f" * 64,),
    ).execute(request_id="request:ambiguous-consume", effect=effect)

    assert receipt.outcome is EffectOutcome.AMBIGUOUS
    assert receipt.authority_receipt_id == authority_evidence[0]
    assert receipt.evidence == tuple(sorted(authority_evidence + ("controller-runtime:sha256:" + "f" * 64,)))
    terminal.assert_called_once_with(receipt.sealed_mapping())
    assert all(provider.call_count == 0 for provider in providers.values())
...
def test_bootstrap_effect_rejects_host_path_command_digest_and_cas_broadening_before_consumption():
    ...
    changes = (
        {"target_host": "other"},
        {"candidate_commit": SHA},
        {"bootstrap_contract_ref": "symbolic:latest"},
        {"bootstrap_contract_hash": "sha256:" + "0" * 64},
    ...
    for change in changes:
        ...
        with pytest.raises(ValueError):
            AuthorityEffectController(registry, authority).execute(
                ...
                effect=TypedEffect.parse(effect),
                executor_principal=EXECUTOR,
            )
...
def test_bootstrap_effect_without_an_exact_contract_resolver_holds_before_authority_consumption():
    ...
    with pytest.raises(ValueError, match="resolver is not mounted"):
        AuthorityEffectController(registry, authority).execute(
            request_id="bootstrap:unmounted",
            effect=effect,
            executor_principal=EXECUTOR,
        )
    authority.begin_execution.assert_not_called()
