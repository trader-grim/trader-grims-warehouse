...
@pytest.mark.parametrize(
    ("operation", "invoke"),
    [
        ("Plan brief retrieval", lambda: mcp_server.tgw_get_plan_brief("PP-CONTEXT-001")),
        ("Plan Graph retrieval", lambda: mcp_server.tgw_get_plan_graph("context")),
    ],
)
def test_projection_runtime_routes_plan_reads_to_canonical_context_service(
    monkeypatch, operation, invoke,
):
    monkeypatch.setattr(mcp_server, "_cfg", {
        "plan_projection_path": "/opt/TGW/current/agent-services/plan-runtime/projection.json",
        "plan_approved_commit": "f" * 40,
        "plan_approved_solution_hash": "sha256:" + "a" * 64,
        "plan_context_service": "tgw-context",
    ...
    assert result["error"]["code"] == "CANONICAL_PLAN_CONTEXT_REQUIRED"
    assert operation in result["error"]["message"]
    assert result["context_service"] == "tgw-context"
