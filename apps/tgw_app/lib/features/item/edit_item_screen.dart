...
class _EditItemScreenState extends ConsumerState<EditItemScreen> {
  ...
  void initState() {
    super.initState();
    _titleController =
        TextEditingController(text: widget.item.data['title'] ?? '');
    _priceController = TextEditingController(
        text: widget.item.data['price']?.toString() ?? '');
    _hintController =
        TextEditingController(text: widget.item.data['ai_hint'] ?? '');
  ...
  void _initializeAspects() {
    final existingAspects =
        widget.item.data['draft_listing']?['aspects'] as Map? ?? {};
  ...
  Future<void> _fetchAspects() async {
    ...
    try {
      final aspects =
          await ref.read(repositoryProvider).getEbayAspects(categoryId);
      if (mounted) {
        setState(() {
          _aspects = aspects;
  ...
  Future<void> _save() async {
    ...
    final success =
        await ref.read(repositoryProvider).patchItem(widget.item.sku, fields);
  ...
  Future<void> _performAction(String action) async {
    final jobId = await ref
        ...
        .performAction(widget.item.sku, action);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(jobId != null
                ? 'Action queued: $action (Job #$jobId)'
                : 'Action failed')),
      );
    }
  }
  ...
  Future<void> _confirmAndPerformAction(
      String action, String label, String description) async {
        ...
        children: [
          ...
          if (mode == 'SELECTION_ONLY')
            DropdownButtonFormField<String>(
              initialValue:
                  values.any((v) => v['localizedValue'] == controller.text)
                      ? controller.text
                      : null,
              items: [
                ...
                ...values.map((v) => DropdownMenuItem(
                      ...
                      child: Text(v['localizedValue'] as String,
  ...
  Future<void> _uploadToInbox() async {
    ...
    final filename =
        await ref.read(repositoryProvider).uploadToInbox(File(path));
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(filename != null
            ? 'Uploaded to inbox: $filename'
            : 'Upload failed'),
  ...
  Future<void> _showTitleHistory() async {
    ...
    try {
      final items = await ref
          .read(repositoryProvider)
          .getItems(search: search, limit: 10);
      if (mounted) {
        setState(() => _isSaving = false);
        final titles = items.map((e) => e.title).toSet().toList();
        if (titles.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('No matching titles found')));
          return;
        }
            ...
            content: SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) => ListTile(
                  title:
                      Text(titles[index], style: const TextStyle(fontSize: 14)),
                  onTap: () {
        ...
        child: Column(
          ...
          children: [
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    items: _conditions
                        .map((c) => DropdownMenuItem(
                            value: c, child: Text(c.isEmpty ? 'None' : c)))
                        .toList(),
                    onChanged: (val) =>
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'AI Hint', border: OutlineInputBorder()),
              maxLines: 3,
            ),
            const SizedBox(height: 32),
            const Text('Item Specifics (Aspects)',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            if (_isLoadingAspects)
              const Center(child: CircularProgressIndicator())
            else if (_aspects.isEmpty)
              const Text('No aspects found for this category.',
