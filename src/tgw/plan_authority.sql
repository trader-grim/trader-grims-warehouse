...
CREATE TABLE IF NOT EXISTS plan_authority_effect_receipts (
    ...
    outcome text CHECK (outcome IN ('succeeded','retry','hold','ambiguous','rolled_back','failed','legacy-consumed')),
...
ALTER TABLE plan_authority_effect_receipts
    ADD CONSTRAINT plan_authority_effect_receipts_outcome_check CHECK (
        outcome IN ('succeeded','retry','hold','ambiguous','rolled_back','failed','legacy-consumed') OR outcome IS NULL
    );
