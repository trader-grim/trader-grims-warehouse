...
def audit_environment_recovery(root: Path, *, observed_at: str) -> dict[str, Any]:
    ...
    def task_context_check() -> str:
        ...
        repeated = resolve_task_context(task, registry)
        if actual != repeated:
    ...
    def steward_check() -> str:
        ...
        return "clean source contract has no production effects or satellite runtime dependency"
    ...
    server_checks = [
        ...
        _check("task-context-reproducible", "config/environment/tasks/environment-recovery.json", task_context_check),
        _check("clean-steward-boundary", "config/environment/actors/tgw-steward.json", steward_check),
