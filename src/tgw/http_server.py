...
_CATEGORY_PICKER_JS = """
function saveDraftCommand(fields){
  return fetch('/api/operator/items/'+window._ITEM_SKU,{headers:authHeaders()})
    .then(function(r){return r.json();})
    .then(function(o){
      if(!o||!o.ok||!o.object)return {ok:false,detail:'operator object unavailable'};
      return fetch('/api/operator/items/'+window._ITEM_SKU+'/commands',{
        method:'POST',
        headers:authHeaders({'Content-Type':'application/json'}),
        body:JSON.stringify({command_id:'save-listing-draft',object_generation:o.object.object_generation,values:{draft_listing:fields}})
      }).then(function(r2){return r2.json();});
    });
}
function detailText(d){return (d&&d.detail)?(typeof d.detail==='string'?d.detail:JSON.stringify(d.detail)):'failed';}
...
function initCatPicker(cfg){
  ...
  function selectCategory(cid,cname){
    ...
    if(cfg.primary)window._DL_CAT_NAME=cname;
    var patch={};
    patch[cfg.idField]=cid;
    patch[cfg.nameField]=cname;
    saveDraftCommand(patch).then(function(d){
      if(d&&d.ok&&cfg.primary&&typeof loadCatCtx==='function')loadCatCtx(cid);
    });
  }
...
def _render_item_detail_html(
    ...
    actions_html = (
        ...
        f"  if(msg)msg.textContent='Saving…';"
        f"  saveDraftCommand(dl).then(function(d){{"
        f"    if(!d.ok){{"
