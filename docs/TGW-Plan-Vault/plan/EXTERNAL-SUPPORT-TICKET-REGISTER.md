...
| `EBAY-DS-1590` | **New ticket** (not a reply to `260605-000035`, which was closed): re-request the EPS increase that case never answered, plus ask eBay to name a concrete "alternative option" for sold/comp price data per their own denial-letter offer. | _(none yet — not submitted)_ | `prepared / not yet submitted` | Draft ready: `reference/DRAFT-1590-ebay-alternative-options-followup.md`, todo #1590. Deliberately filed as a standalone ticket, not a follow-up-in-thread, so it can't be silently closed-by-association with an unrelated answer the way `260605-000035` was. Once submitted, record eBay's assigned case number as the External reference. 2026-07-20: Dave separately probed eBay's AI-generated support chat (not the formal ticket system) — it confirmed no item-level sold-price API/report exists for independent sellers (Marketplace Insights genuinely has no substitute), and stated the Application Growth Check is the only documented path to an EPS increase, implying one hadn't been done — but `260605-000035` was itself a Growth Check with the EPS ask bundled in, so this is either the bot lacking case-history context or further confirmation only the Marketplace Insights half was ever processed. Draft ticket text now explicitly states the check already happened under that case number. |
