...
| `EBAY-DS-260605-000035` | **Application Growth Check.** Existing App Check / `buy.marketplace_insights` scope request, now including the EPS / `UploadSiteHostedPictures` daily call-limit increase request (taskboard #1076), plus a follow-up requesting the "alternative options" eBay offered (todo #1590). | `260605-000035` | `response-received (denial) / follow-up drafted` | 2026-07-20: eBay formally **denied** `buy.marketplace_insights` ("highly limited, reserved for approved partners") — closes the 8-open-questions state, this half of the case is resolved (denied, not pending). Their denial closed with an offer to discuss "alternative options that might fit your use case" — follow-up drafted (`reference/DRAFT-1590-ebay-alternative-options-followup.md`, todo #1590) asking them to name a concrete alternative, **not yet sent**. Separately, the EPS increase request never got a response of its own — **live-reverified 2026-07-20** via a real `getRateLimits` call: `UploadSiteHostedPictures` limit is still exactly 5,000/day, unchanged since the 2026-07-02 baseline, confirming eBay did not silently grant/change it. Follow-up (#1590) re-presses this in the same message. Prepared attachment: `/home/db/Sync/ebay-dev-support-eps-limit-increase.txt`. Source: taskboard #79 and #1076; `reference/DRAFT-1076-eps-support-ticket.md`; `reference/DRAFT-1590-ebay-alternative-options-followup.md`. |
