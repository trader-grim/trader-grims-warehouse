...
“Done” must not mean an untraceable overwrite of catalog truth. It means a
complete, idempotent capture package with target identity, raw photo references,
measurement fields/units, timestamps, actor/device/probe context, and the
requested create-or-update intent. The catalog/workflow authority can then apply
or review the package according to its existing policy.
