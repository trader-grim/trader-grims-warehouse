...
capture path: a basic photo/measurement/update-packet flow should work without
privileged operations, while root/custom-ROM-only enhancements are explicitly
identified and tested. This remains a candidate hardware path, not yet a
dependency assumption. Its first fixture should prove how the scanner presents
data (keyboard/HID wedge, Android intent, vendor API, or another route), then
bind a successful scan to existing-SKU target selection without changing the
