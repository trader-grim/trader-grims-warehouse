...
System framing:
  the resulting unit is not “Tasker plus a Linux console.” It is an integrated
  data-collection probe with a two-way event surface:

  command/intent ingress → validated local action
  collection/health/result egress → HUD and chosen upstream consumer

  Tasker, a Linux command, or another Android automation tool may remain an
  internal adapter where useful; none is the user-facing or architectural
  center.
```
...
**First product workflow — create or enrich an item package:**

The same probe should support two target modes rather than becoming two apps:

```text
NEW ITEM
  identify/start a new item package → capture required photos → record
  measurements → finish → produce a staged, provenance-linked intake package.

EXISTING SKU
  select/scan the SKU → capture another photo or a small photo set → record
  measurements → finish → produce a staged evidence/update package linked to
  that SKU.
```

The fast operator experience Dave wants is deliberately small:
...
“Done” must not mean an untraceable overwrite of catalog truth. It creates the
**update packet** that flows through the existing pipeline to propose/apply the
catalog update. Capture and delivery are separate from catalog authority.

```text
capture complete
  → durable local update packet + photo/measurement manifest
  → if connected: send immediately
  → if disconnected: retain in a local outbox and forward on reconnection
  → upstream receipt/acknowledgement
  → pipeline validation/review/application under existing policy
```

The packet carries target identity, raw photo references (and their transfer
status), measurement fields/units, timestamps, actor/device/probe context, and
the requested create-or-update intent. It needs a stable packet identity so
retries/resends cannot silently create duplicate updates. Local source material
must remain recoverable until receipt is acknowledged; the HUD should distinguish
captured, queued, sent, acknowledged, rejected, and failed—not collapse these
into a premature “done.”

**Standalone first; integration is an adapter:**

The camera application must remain useful as a standalone local capture tool.
Network/pipeline integration enriches it but must not be required for a user to
capture photos, measurements, and a complete local update packet. Its local
outbox is therefore a real product surface, not merely a failure fallback.

The JSON envelope may carry arbitrary additional key/attribute pairs so the
probe can collect useful evidence without a camera-app release for every new
field. Preserve a small reserved core (packet identity, schema/version, target,
intent, timestamps, delivery state, media/measurement references) and keep
extension fields namespaced or template-defined so flexible attributes remain
interpretable downstream.

**Planned hardware context — custom-ROM Pixel plus barcode scanner:**

The probe is likely to live on a Pixel running a custom ROM with a supporting
hardware package that includes a hardware barcode scanner. Pixel is the chosen
family because it is the most reliable target for the Kotlin development kits,
while older Pixel 7/8-generation devices remain practical to acquire and have
the same camera architecture Dave needs from newer phones. On unlocked,
Dave-controlled devices, custom-ROM and root capability are available when a
fixture genuinely needs them.

Those capabilities are enablers, not requirements to impose on the normal
capture path. The currently named root-only exception is a **Foldio360
workaround** needed to make the intended turntable/camera flow usable. Root is
not a general privilege for the probe: it is limited to that documented
hardware-integration workaround unless a later reviewed fixture establishes
another need.

A turntable alternative remains an explicit decision path. Compare the
Foldio360 workaround and any viable alternative against the same bounded
fixture—reliable control, capture/rotation synchronization, repeatable output,
recoverability, and operator friction—then choose the smallest solution that
works. The basic photo/measurement/update-packet flow must still work without
turntable control.

**Candidate hardware-research path — Ortery USB protocol:**

Dave has begun looking at reverse engineering the USB interaction to the Ortery.
The first requirement is a known-good baseline: locate a Windows PC capable of
running the original supplied software with the hardware attached. Treat that
PC/software path as a temporary observation fixture, not a permanent Windows
dependency.

```text
1. Preserve the original installer/media, version, documentation, and hashes.
2. Record device identity/enumeration and a clean known-good software session.
3. Capture USB traffic while performing small, deliberate, safe UI actions.
4. Correlate each UI action with device response/state before inferring protocol.
5. Separate read/identify/status behavior from movement/capture commands.
6. Do not replay physical-control commands against live equipment until a
   bounded safety/recovery fixture exists.
```

A VM may later be useful only if the original driver/software and reliable USB
passthrough are demonstrated. A physical Windows PC is the simplest first
baseline. The Ortery is a proven professional-quality device Dave used years
ago; it was displaced by Foldio360 only because an earlier rooted Droid 2 setup
made the Foldio workflow faster, not because the Ortery was unsuitable.

The research question is therefore narrow: recover the device's **positional
language**. It exposes roughly 1,400 or more possible positions, so the native
adapter must not reduce it to an assumed "rotate N degrees" abstraction. First
determine whether the original software uses position IDs, a calibrated
coordinate/index, relative steps, home/reference commands, or another encoding;
preserve the original device representation alongside any friendly capture
labels.

The current working hypothesis is that the positional language may be very
small despite the 1,400+ addressable positions:
...
Possibly accompanied only by identity/status/reference behavior. Treat this as
a hypothesis, not a recovered protocol. The discriminating observation fixture
is small: issue the same known target once in each direction, observe a stop
mid-travel, test an already-current target, and record any reported completion
or position state. If that pattern holds, this is a narrow command-decoding task
rather than a large controller reimplementation.

A useful protocol/capture contract will distinguish:

```text
requested position
→ device command / acknowledgement
→ movement complete or failure
→ observed/returned position (if available)
→ camera capture bound to that confirmed position
```

Start with a small set of known-safe reference operations (identity/status,
home/reference if the original workflow proves it, one small movement, and one
known target) and retain both the USB trace and physical/result observation.
That permits incremental decoding without guessing the whole 1,400-position
space. Findings can either support a narrow native adapter or demonstrate that
a different turntable is the lower-risk choice. This remains a candidate
hardware path, not yet a dependency assumption. Its first fixture should prove
how the scanner presents data (keyboard/HID wedge, Android intent, vendor API,
or another route), then bind a successful scan to existing-SKU target selection
without changing the update-packet contract. The camera/measurement flow must
remain usable if scanner hardware is unavailable.

A category/template may determine the required measurement fields and photo
minimums. Keep those defaults declarative and configurable; do not hardcode a
single item-type assumption into the camera probe.
