...
turntable control. This remains a candidate hardware path, not yet a dependency
assumption. Its first fixture should prove how the scanner presents data
(keyboard/HID wedge, Android intent, vendor API, or another route), then bind a
successful scan to existing-SKU target selection without changing the
update-packet contract. The camera/measurement flow must remain usable if
